...
COURSE_OPTIONS = [
    # EXTENSÃO DE CÍLIOS T1.1 — Pontal Country Clube
    {"id": "11", "curso_id": "01", "curso": "EXTENSÃO DE CÍLIOS", "turma": "TURMA 1.1",
     "local": "PONTAL COUNTRY CLUBE",
     "dias_aula": "Sábados — 12/09, 19/09 e 26/09",
     "horario": "9h às 16h",
     "data_inicio": "12/09/2026", "encerramento": "26/09/2026",
     "endereco_curso": "📍Estrada do Pacuí, 200 — Vargem Grande — Rio de Janeiro"},
]
COURSE_OPTIONS_BY_ID = {option["id"]: option for option in COURSE_OPTIONS}
COURSE_INFO = COURSE_OPTIONS[0]
WHATSAPP_SHARE_HOME_URL = "https://mulheres-do-corre-8.onrender.com"
def build_whatsapp_share_url():
    message = (
        "Acabei de me inscrever no programa Mulheres do Corre 8, com cursos gratuitos de "
...
<head>
    ...
    <title>Mulheres do Corre 8</title>
...
<body data-start-step="{{ current_step }}">
            ...
            <form id="wizard-form" method="POST" action="{{ url_for('inscricao_unica') }}" autocomplete="off" novalidate>
                <section class="wizard-panel" data-step="index">
                    <div class="hero-grid"><div class="hero-card">
                        ...
                        <h1 class="hero-title">MULHERES DO CORRE 8</h1>
                ...
                <section class="wizard-panel" data-step="escolher">
                    <div class="step-card"><h2 class="panel-title">Informa&#231;&#245;es do curso</h2>
                        <div class="step-grid step-grid--stacked">
                    ...
                    <div class="step-card"><h2 class="panel-title">Revise antes de finalizar</h2>
                        ...
                        <div class="review-layout">
                            ...
                            <div class="review-box full">
                                <div style="margin-bottom:10px;color:#7a2520;font-size:0.98rem;text-align:left;"><strong>Elegibilidade:</strong> Esta inscri&#231;&#227;o &#233; destinada a pessoas interessadas nos cursos de qualifica&#231;&#227;o profissional do programa Mulheres do Corre 8.</div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function syncReview(){reviewTargets.forEach(function(target){var field=document.getElementById(target.dataset.review);if(!field){target.textContent='';return;}if(field.tagName==='SELECT'){var sel=field.options[field.selectedIndex];target.textContent=sel?sel.text.trim():'';}else{target.textContent=field.value.trim();}});}
...
def validate_form_data(form_data):
    ...
    if not selected_option and form_data.get("curso") and form_data.get("local"):
        match=next((o for o in COURSE_OPTIONS if o["curso"]==form_data["curso"] and o["local"]==form_data["local"]),None)
        if match:
            form_data["opcao_id"]=match["id"]
            selected_option=match
    if not selected_option: errors["opcao_id"]="Selecione uma turma v\u00e1lida."
    nome=form_data["nome"]
...
def confirmacao(protocolo=None):
    if not protocolo: protocolo=session.get("protocolo")
